...
const SalesInvoice = () => {
  ...
  return (
    <Container maxWidth={false} sx={{ mt: 1, mb: 1, px: { xs: 1, sm: 2, md: 3 }, height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column' }}>
      <Paper
        elevation={3}
        sx={{
          p: { xs: 1.5, sm: 2, md: 2 },
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2, mb: 2 }}>
          ...
          >
            {showAddForm ? 'Create Sales Invoice' : showReturnForm ? 'Return Invoice' : 'Sales Invoices'}
          </Typography>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
            <Button
              variant="contained"
              startIcon={showAddForm || showReturnForm ? null : <AddIcon />}
              onClick={() => { setShowAddForm(!showAddForm); if (showReturnForm) setShowReturnForm(false); }}
              sx={{
                background: gradients.primary,
            ...
            >
              {showAddForm || showReturnForm ? 'Back to List' : 'Create New'}
            </Button>
            {!showAddForm && !showReturnForm && (
              <Button
                variant="contained"
                ...
                sx={{
                  ...
                  '&:hover': { background: gradients.warningHover },
                }}
              >
                Return Invoice
              </Button>
        ...
        {formError && (
          <Box sx={{ mb: 2, p: 1.5, background: 'rgba(239, 68, 68, 0.1)', border: '1px solid #fecaca', borderRadius: 1 }}>
        ...
        {showAddForm ? (
          <>
            {/* Top row: Party, Date, Sauda Summary, Total Fine - sticky */}
            <Grid container spacing={2} sx={{ mb: 2, position: 'sticky', top: 0, zIndex: 2, bgcolor: 'background.paper', pt: 1 }}>
          ...
          <Grid item xs={12} sm={6} md={6}>
            <Box sx={{ p: 1.5, background: gradients.primary, borderRadius: 2, color: '#fff', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Typography variant="body2" sx={{ opacity: 0.9 }}>Total Fine:</Typography>
              <Typography variant="h6" sx={{ fontWeight: 700 }}>{getTotalFine()} g</Typography>
            </Box>
          </Grid>
        </Grid>
            ...
            <TableBody>
              {filteredPagga.map((pagga, index) => {
                ...
                return (
                  <TableRow
                    key={pagga._id}
                    onClick={() => handleCheckPagga(pagga._id)}
                    sx={{ cursor: 'pointer' }}
                  >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, pt: 1, borderTop: '1px solid', borderColor: 'divider' }}>
          ...
          <>
            <Grid container spacing={2} sx={{ mb: 2, position: 'sticky', top: 0, zIndex: 2, bgcolor: 'background.paper', pt: 1 }}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                <Box sx={{ p: 1.5, background: gradients.warning, borderRadius: 2, color: '#fff', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Typography variant="body2" sx={{ opacity: 0.9 }}>Total Fine:</Typography>
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>{getReturnTotalFine()} g</Typography>
                </Box>
              </Grid>
            </Grid>
                ...
                <TableBody>
                  {returnAvailablePagga
                    ...
                    return (
                      <TableRow
                        key={pagga._id}
                        onClick={() => handleReturnPaggaToggle(pagga._id)}
                        sx={{
                          cursor: 'pointer',
                          '&:hover': {
        ...
        >
          ...
          <DialogContent>
            ...
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow sx={{ background: gradients.successDark }}>
                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Sr No</TableCell>
                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Pagga No</TableCell>
                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Weight (g)</TableCell>
                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Touch (%)</TableCell>
