          ...
          const StatCard = ({ title, value, subtitle, icon, color }) => (
            <Card
              sx={{
                ...
                borderTop: theme.palette.mode === 'dark' ? '1px solid rgba(255,255,255,0.05)' : 'none',
                ...
                boxShadow: theme.palette.mode === 'dark' ? '0 4px 20px rgba(0,0,0,0.5)' : '0 4px 12px rgba(0,0,0,0.05)',
                '&:hover': {
                  transform: 'translateY(-2px)',
                  boxShadow: theme.palette.mode === 'dark' ? '0 6px 24px rgba(0,0,0,0.5)' : '0 6px 16px rgba(0,0,0,0.08)',
            ...
            >
              <CardContent sx={{ p: { xs: 1.5, sm: 2, md: 2.5 } }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                  <Typography
                    variant="subtitle2"
                    sx={{ color: 'text.secondary', fontWeight: 600, fontSize: { xs: '0.75rem', sm: '0.85rem', md: '0.9rem' } }}
                  >
                    {title}
                  </Typography>
                ...
                <Typography
                  variant="h5"
                  sx={{ fontWeight: 700, color: 'text.primary', mb: 0.5, fontSize: { xs: '1.05rem', sm: '1.25rem', md: '1.45rem' } }}
                >
                  {loadingStock ? '...' : value}
                </Typography>
          ...
          return (
            <Grid container spacing={3} sx={{ mt: 1 }}>
              <Grid item xs={12} sm={4}>
                <StatCard 
                  title="Total Buy"
                  value={`+${Number(buyW).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}g`}
                  subtitle="Purchased"
              ...
              <Grid item xs={12} sm={4}>
                <StatCard 
                  title={isBank ? "Total Sell" : "Total Sell + Badla"}
                  value={`-${Number(sellW).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}g`}
                  subtitle="Sold & Given"
                  icon={<SellIcon />}
                  color="#ef4444"
                />
              ...
              <Grid item xs={12} sm={4}>
                <StatCard 
                  ...
                  value={`${Number(netStock).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}g`}
