...
export async function printPartyLedger(party, entries, summary, pendingSaudas, startDate, endDate) {
  ...
  const getTypeLabel = (entry) => {
    if (entry.type === 'payment') {
      ...
      return 'Credit Note';
    } else if (entry.type === 'debit-note') {
      return 'Debit Note';
    }
    return '-';
  };

  // Function to render entries for a specific type
  const renderEntriesByType = (typeEntries, type) => {
    ...
    typeEntries.forEach((entry) => {
      ...
      if (entry.type === 'payment') {
        particulars = (entry.paymentType === 'incoming' ? 'Payment In' : 'Payment Out') +
          (entry.paymentNo ? ' - ' + entry.paymentNo : '') + (entry.remark ? ' (' + entry.remark + ')' : '');
        fineStr = '-';
      } else if (entry.type === 'Purchase') {
        ...
        particulars = 'Credit Note' + (entry.noteNo ? ' - ' + entry.noteNo : '') + (entry.reason ? ' (' + entry.reason + ')' : '');
        fineStr = entry.fine ? entry.fine + ' g' : '-';
      } else if (entry.type === 'debit-note') {
        particulars = 'Debit Note' + (entry.noteNo ? ' - ' + entry.noteNo : '') + (entry.reason ? ' (' + entry.reason + ')' : '');
        fineStr = entry.fine ? entry.fine + ' g' : '-';
      }
  ...
  htmlContent += `
          <tr class="totals-row">
            ...
            <td class="num">${closingBal !== 0 ? (closingBal > 0 ? closingBal + ' (dena hai)' : closingBal + ' (lena hai)') : closingBal}</td>
          </tr>
        </tbody>
      </table>
  `;
...
export function printCashBook(cashBook) {
  const formatDate = (d) => d ? new Date(d).toLocaleDateString('en-GB') : '-';
  ...
  transactionsWithBalance.forEach((t) => {
    rowsHtml += `
      <tr>
        <td class="center">${formatDate(t.date)}</td>
        <td>${t.particular}${t.remark ? ' (' + t.remark + ')' : ''}</td>
        <td class="center">${t.voucherType}</td>
        <td class="center">${t.voucherNo}</td>
        <td class="num">${t.debit > 0 ? Math.trunc(t.debit) : ''}</td>
        <td class="num">${t.credit > 0 ? Math.trunc(t.credit) : ''}</td>
        <td class="num bold">${Math.trunc(t.balance)}</td>
      </tr>
    `;
  });
      ...
      <table class="ledger-table">
        ...
        <tbody>
          ...
          <tr class="totals-row">
            ...
            <td class="num">${Math.trunc(totalIncoming)}</td>
            <td class="num">${Math.trunc(totalOutgoing)}</td>
